...
export class ProductoService {
    ...
    public productosreturn() {
        return this.productos;
    }

    public productofindID(id) {
        return this.productos.find(function (productoid) {
            if (productoid.productId == id) {
                return productoid
            }
        })
    }

    public Buscarproductos(busqueda) {
        busqueda = busqueda.toLowerCase();
        return this.productos.filter((producto) => producto.productName.toLowerCase().indexOf(busqueda) > -1)
    }
